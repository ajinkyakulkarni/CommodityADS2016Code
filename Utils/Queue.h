...
template<typename ITEM> class Queue
{
    ...
    ITEM* items;
    int offset(int i){return (front + i) % capacity;}
    void resize()
    {
        ITEM* oldArray = items;
...
public:
    ...
    Queue(Queue const& rhs): capacity(rhs.capacity), size(rhs.size),
        front(rhs.front), items(rawMemory<ITEM>(capacity))
    {
        for(int i = 0; i < size; ++i)
            new(&items[i])ITEM(rhs.items[offset(i)]);
    }
