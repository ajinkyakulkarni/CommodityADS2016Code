...
struct DecisionTree
{
    ...
    template<typename DATA> struct Comparator
    {
        ...
        bool isLess(int lhs, int rhs)const{return v(lhs) < v(rhs);}
        ...
        int nClasses, double pruneZ, int depth, bool rfMode)
    {
        ...
        //try to prune
        int treeAccuracy = lData.second + rData.second,
            nTreeWins = treeAccuracy - nodeAccuracy, nDraws = n - nTreeWins;
        if(signTestAreEqual(nDraws/2.0, nDraws/2.0 + nTreeWins, pruneZ))
        {
            rDelete(node);
...
class BinaryLSVM
{
    ...
    double f(NUMERIC_X const& x)const{return w * x + b;}
    template<typename DATA> struct GSL1Functor
    {
        DATA const& data;
        Vector<double> sums;
        double l;
        ...
        {
            double result = j < 0 ? 0 : l * abs(wj);
            for(int i = 0; i < data.getSize(); ++i) result += loss(sums[i] +
                (j < 0 ? 1 : data.getX(i, j)) * wj, y(data.getY(i)));
            return result/data.getSize();
        }
    };
    template<typename DATA>void coordinateDescent(DATA const& data, double l,
        int maxIterations, double eps)
    {
        int D = getD(data);
        GSL1Functor<DATA> f =
            {data, Vector<double>(data.getSize(), b), l, -1};
        for(int i = 0; i < data.getSize(); ++i)
            for(int j = 0; j < D; ++j)
                f.sums[i] += w[j] * data.getX(i, j);
        bool done = false;
        Vector<double> prevDiffs(D + 1, 1);
        while(!done && maxIterations--)
        {
            ...
            for(int j = -1; j < D; ++j)
            {
                double &wj = (j < 0 ? b : w[j]);
                for(int i = 0; i < data.getSize(); ++i)
                    f.sums[i] -= j < 0 ? b : w[j] * data.getX(i, j);
                f.j = j;
                double& pd = prevDiffs[j < 0 ? D : j];
                double left = findIntervalBound(f, wj, -pd),
                    right = findIntervalBound(f, wj, pd);
                if(right - left > eps)
                {
                    double wjNew = minimizeGS(f, left, right, eps/10).first;
                    pd = abs(wj - wjNew);
                    if(pd > eps) done = false;
                    wj = wjNew;
                }
                for(int i = 0; i < data.getSize(); ++i)
                    f.sums[i] += j < 0 ? b : w[j] * data.getX(i, j);
...
public:
        ...
        params, int maxRepeats = 10, int maxConst = 10000): K(params.first)
    {
        ...
        for(int k = 0; k < n; ++k) if(abs(d[k]) > globalSafeDelta)
            {
...
public:
    template<typename DATA> SimpleBestCombiner(DATA const& data)
    {
        ...
        c.addNoParamsClassifier<AdaBoostSamme<> >(data, SCVRiskFunctor<
            NoParamsLearner<AdaBoostSamme<>, int>, EMPTY, DATA>(data));
        c.addNoParamsClassifier<SSVM>(data, SCVRiskFunctor<
            NoParamsLearner<SSVM, int>, EMPTY, DATA>(data));
        c.addNoParamsClassifier<SNN>(data, SCVRiskFunctor<
            NoParamsLearner<SNN, int>, EMPTY, DATA>(data));
    }
